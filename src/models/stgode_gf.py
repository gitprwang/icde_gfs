    ...
    def __init__(self, A_sp, A_se, **args):     
        ...
        self.sp_blocks = nn.ModuleList(
            [nn.Sequential(
                ...
                node_num=self.node_num, A_hat=None), 
                STGCNBlock(in_channels=64, out_channels=[64, 32, 64],
                node_num=self.node_num, A_hat=None)) for _ in range(3)
            ])
...
class ODEFunc(nn.Module):
    def __init__(self, feature_dim, temporal_dim, adj):
        ...
        self.adj = adj
        self.x0 = None
        self.alpha = nn.Parameter(0.8 * torch.ones(8600))  # adj.shape[1]
        self.beta = 0.6
        ...
        self.gcn = GFS(feature_dim, feature_dim)
    ...
    def forward(self, t, x):
        ...
        xa = self.gcn(x.permute([0,2,1,3])).permute([0,2,1,3])
