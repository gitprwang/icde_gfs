...
class spatialAttentionScaledGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        global gcn_att_list
        if record and len(gcn_att_list)<=80:
            gcn_att_list.append(spatial_attention.detach().cpu().numpy())
        if len(gcn_att_list)>80:
            gcn_att_list.clear()
...
class LN(nn.Module):
    def __init__(self, f_dim, node=307):
        super(LN, self).__init__()
        self.norm = nn.LayerNorm([node, f_dim])
    def forward(self, x):
        # x : B,N,T,d
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, trg):
        ...
        return self.decode(trg, encoder_output).permute(0,2,1,3)
...
def make_model(DEVICE, num_layers, encoder_input_size, decoder_output_size, d_model, adj_mx, nb_head, num_of_weeks,
    ...
    norm_Adj_matrix = torch.from_numpy(adj_mx).type(torch.FloatTensor).to(DEVICE) # torch.from_numpy(norm_Adj(adj_mx)).type(torch.FloatTensor).to(DEVICE)  # 通过邻接矩阵，构造归一化的拉普拉斯矩阵adj_mx.type(torch.FloatTensor).to(DEVICE) #
    ...
    #     position_wise_gcn = GraphMLP(d_model, d_model, num_nodes=num_of_vertices) # GraphMLP PositionWiseGCNFeedForward
    # else:  # 不带attention
    #     position_wise_gcn = GraphMLP(d_model, d_model, num_nodes=num_of_vertices) # GraphMLP PositionWiseGCNFeedForward
