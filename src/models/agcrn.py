...
class AGCRNCell(nn.Module):
    def __init__(self, dim_in, dim_out, cheb_k, embed_dim):
        ...
        self.gate = GFS(dim_in+self.hidden_dim, 2*dim_out, cheb_k, embed_dim) # AVWGCN
        self.update = GFS(dim_in+self.hidden_dim, dim_out, cheb_k, embed_dim) # AVWGCN
...
class GFS(nn.Module):
    def __init__(self, dim_in, dim_out, cheb_k=None, embed_dim=None, rank=1, affine=True):
        ...
        self.node_weight = nn.Parameter(torch.ones(rank, 8600)/8600., requires_grad=True)
        self.add_weight = nn.Parameter(torch.ones(8600, rank)/8600, requires_grad=True)
        if self.affine:
    ...
    def forward(self, x, node_embeddings=None):
        ...
        x = torch.einsum('acd,de->ace', x, self.w)
        x = self.act(x + self.b)
        x = torch.einsum('ef,fe,aed->aed', self.add_weight, self.node_weight, x)
        if self.affine:
